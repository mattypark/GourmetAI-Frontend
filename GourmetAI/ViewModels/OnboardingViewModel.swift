...
class OnboardingViewModel: ObservableObject {
    ...
    // MARK: - HealthKit (Q16)
    @Published var enableHealthKitSync: Bool?
    ...
    // Flow: 17 questions (Q0-Q16)
    //
    // Section 1 - Personal Info:
    //   0: Name, 1: Gender, 2: Birthday, 3: Height, 4: Weight, 5: Desired Weight, 6: Activity Level
    //
    // Section 1b - HealthKit (conditional):
    //   16: Connect Apple Health? (only shown if HealthKit available on device)
    //
    ...
    let questions: [OnboardingQuestion] = [
        ...
        // HealthKit (shown after Q6 if available)
        OnboardingQuestion(id: 16, title: "Connect Apple Health?", subtitle: "Sync your health data to personalize nutrition goals and log meals you cook.", type: .singleChoice(["Yes, connect", "Not now"]))
    ]
    ...
    var visibleQuestionIndices: [Int] {
        ...
        // Q0-Q6: Personal info (always visible)
        indices.append(contentsOf: [0, 1, 2, 3, 4, 5, 6])

        // Q16: HealthKit (only if available on device)
        if HealthKitService.shared.isAvailable {
            indices.append(16)
        }

        // Q7-Q12: Cooking habits + motivation
        indices.append(contentsOf: [7, 8, 9, 10, 11, 12])
    ...
    var canProceed: Bool {
        ...
        case 16: return enableHealthKitSync != nil
    ...
    func proceedFromCurrentQuestion() {
        // Handle HealthKit authorization when user says "Yes"
        if currentQuestionIndex == 16 && enableHealthKitSync == true {
            Task {
    ...
    func completeOnboarding() {
        // OWASP: Sanitize and enforce length limit on user-supplied name
        let sanitizedName = String(userName.trimmingCharacters(in: .whitespacesAndNewlines).prefix(50))
        userProfile.userName = sanitizedName.isEmpty ? nil : sanitizedName
        ...
        UserDefaults.standard.set(sanitizedName, forKey: "settings.userName")

        // Save HealthKit preference
        if enableHealthKitSync == true {
            UserDefaults.standard.set(true, forKey: StorageKeys.healthKitOnboardingChoice)
        }

        storageService.saveUserProfile(userProfile)

        // Also persist a minimal profile row to Supabase so returning users
        // on a fresh install are recognized without repeating onboarding
        Task {
            let profileData = UserProfileData(
                id: nil,
                cookingSkillLevel: nil,
                dietaryRestrictions: userProfile.dietaryRestrictions.map { $0.rawValue },
                timeAvailability: userProfile.timeAvailability?.maxMinutes,
                mainGoal: userProfile.mainGoal?.rawValue,
                createdAt: nil,
                updatedAt: Date()
            )
            try? await SupabaseManager.shared.saveProfile(profileData)
        }
    }
