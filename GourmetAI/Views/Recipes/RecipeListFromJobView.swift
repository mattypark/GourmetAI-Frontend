...
struct RecipeListFromJobView: View {
    ...
    @State private var categoryRecipeId: UUID?
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(isPresented: $showingCategorySheet) {
                if let recipeId = categoryRecipeId {
                    AddToCategorySheet(recipeId: recipeId)
                }
            }
        }
    }
                        ...
                        HStack(spacing: 0) {
                            ...
                            Button {
                                ...
                                Image(systemName: favoriteService.isFavorite(recipe.id) ? "heart.fill" : "heart")
                                    .font(.system(size: 16))
                                    .foregroundColor(favoriteService.isFavorite(recipe.id) ? .red : .gray)
                                    .frame(width: 36, height: 44)
                            }

                            Button {
                                categoryRecipeId = recipe.id
