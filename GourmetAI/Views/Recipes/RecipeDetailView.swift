...
struct RecipeDetailView: View {
    ...
    @State private var showingAddToCategory: Bool = false
    ...
    @State private var showingMealLogged: Bool = false
    @State private var isLoggingMeal: Bool = false
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 24) {
                    ...
                    if let nutrition = recipe.nutritionPerServing, nutrition.hasMacros {
                        ...
                        // Log Meal to Apple Health
                        if healthKitService.isEnabled, nutrition.calories != nil {
                            logMealButton(nutrition)
                        }
            ...
            ToolbarItem(placement: .primaryAction) {
                HStack(spacing: 12) {
                    Button {
                        ...
                        Image(systemName: favoriteService.isFavorite(recipe.id) ? "heart.fill" : "heart")
                            .foregroundColor(favoriteService.isFavorite(recipe.id) ? .red : .black)
                    }

                    Button {
                        showingAddToCategory = true
                    } label: {
        ...
        .sheet(isPresented: $showingAddToCategory) {
            AddToCategorySheet(recipeId: recipe.id)
        }
        .gesture(
            DragGesture()
                .onEnded { value in
    ...
    private func logMealButton(_ nutrition: NutritionInfo) -> some View {
        Button {
            logMealToHealth(nutrition)
        } label: {
            HStack(spacing: 8) {
                if isLoggingMeal {
                    ProgressView()
                        .tint(.white)
                } else if showingMealLogged {
                ...
                Text(showingMealLogged ? "Logged to Apple Health" : "Log Meal to Apple Health")
                    .font(.subheadline)
                    .fontWeight(.semibold)
                    .foregroundColor(.white)
            }
            .frame(maxWidth: .infinity)
            .padding(.vertical, 14)
            .background(showingMealLogged ? Color.green : Color.black)
            .cornerRadius(12)
        }
        .disabled(isLoggingMeal || showingMealLogged)
        .padding(.top, 4)
    }

    private func logMealToHealth(_ nutrition: NutritionInfo) {
        isLoggingMeal = true
        Task {
            do {
                try await healthKitService.logMeal(
...
#Preview {
    RecipeDetailView(
        recipe: Recipe(
            ...
            source: RecipeSource(name: "Gourmet AI Generated", author: "Gourmet AI")
        )
    )
}
