...
struct AnalyzeDishResponse: Codable {
    let success: Bool
    let dishName: String
...
final class APIClient: @unchecked Sendable {
    ...
    // MARK: - Analyze Cooked Dish

    /// Analyzes a photo of a cooked dish and returns the dish name + a full detailed recipe
    func analyzeDish(_ image: UIImage) async throws -> (dishName: String, recipe: Recipe) {
        #if DEBUG
        print("📡 APIClient: Sending dish image to backend for analysis...")
        #endif
        ...
        guard let url = URL(string: "\(baseURL)/api/v1/analyze/dish") else {
            throw APIClientError.invalidURL
        }
        ...
        let apiResponse = try decoder.decode(AnalyzeDishResponse.self, from: data)

        if !apiResponse.success {
            throw APIClientError.serverError(400, apiResponse.message)
        }

        let recipe = apiResponse.recipe.toRecipe()
        #if DEBUG
        print("✅ APIClient: Dish identified as '\(apiResponse.dishName)' with \(recipe.detailedSteps.count) detailed steps")
        #endif

        return (dishName: apiResponse.dishName, recipe: recipe)
    }
    ...
    struct FreeTrialResponse: Codable {
        let success: Bool
        let expiresAt: String
    }

    struct SubscriptionStatusAPIResponse: Codable {
        ...
        let freeTrialExpiresAt: String?
    }

    /// Create a Stripe Checkout Session and return the URL
    func createCheckoutSession(userId: String, plan: String, email: String?) async throws -> String {
        guard let url = URL(string: "\(baseURL)/api/v1/subscriptions/create-checkout") else {
            throw APIClientError.invalidURL
        }
    ...
    /// Start a 24-hour free trial (no credit card)
    func startFreeTrial(userId: String) async throws -> String {
        guard let url = URL(string: "\(baseURL)/api/v1/subscriptions/start-free-trial") else {
            throw APIClientError.invalidURL
        }
        ...
        let trialResponse = try decoder.decode(FreeTrialResponse.self, from: data)
        return trialResponse.expiresAt
    }

    /// Get subscription status for a user
    func getSubscriptionStatus(userId: String) async throws -> SubscriptionStatusAPIResponse {
        guard let url = URL(string: "\(baseURL)/api/v1/subscriptions/status?userId=\(userId)") else {
            throw APIClientError.invalidURL
        }
        ...
        return try decoder.decode(SubscriptionStatusAPIResponse.self, from: data)
    }
